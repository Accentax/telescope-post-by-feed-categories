...
var handleFeed = function(error, feed) {
  ...
  console.log("did the test work? feedId:"+feedId+"categories"+categories)
  ...
  feedItems.forEach(function(item, index, array) {
    ...
    if (!!Posts.findOne({feedItemId: item.id})) {
      ...
      console.log("this is the feed id"+feed.id)
      var post = {
...
fetchFeeds = function() {
  ...
  Feeds.find().forEach(function(feed) {
    ...
    var feedId = feed._id;
    console.log("in fetchfeedsmethod"+feed.categories)
    try {
